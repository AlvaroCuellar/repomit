...
export async function recordVisit(now = new Date()) {
  ...
  await run(`INSERT INTO site_visits (day,total,first_seen) VALUES ($1,1,$2)
    ON CONFLICT(day) DO UPDATE SET total = site_visits.total + 1`,
    [timestamp.slice(0, 10), timestamp]);
}

export async function visitStatistics(now = new Date()) {
  await initialize();
  ...
  const summary = await one<{ total: number | string; first_seen: string | null }>(
    'SELECT COALESCE(SUM(total),0) AS total, MIN(first_seen) AS first_seen FROM site_visits');
  const days = await all<{ day: string; total: number | string }>(
    'SELECT day,total FROM site_visits WHERE day >= $1 AND day <= $2 ORDER BY day DESC', [since, today]);
  return {
